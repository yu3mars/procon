    ...
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            for (int tt = 1; tt <= t; tt++)
            {
                ...
                double d = 100.0 / n;
                double syosuuika = d - (int)d;
                if(syosuuika < eps)
                {
                    ans = 100;
                }
                else if (syosuuika >=0.5)
                {
                    for (int i = 0; i < l; i++)
                    {
                        ans += (int)Math.Round((double)c[i]/n, 0, MidpointRounding.AwayFromZero);
                    }
                    ans += (int)Math.Round((double)1.0 / n, 0, MidpointRounding.AwayFromZero) * (n - ccnt);

                }
                else
                {
                    int shikii = (int)((0.5 - eps) / syosuuika) + 1;
                }
                Console.WriteLine("Case {0}: {1}", tt, n);
            }
        }
    }
}
