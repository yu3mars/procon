...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            long ans = 0;
            long[] adp = new long[n + 2];
            long[] cdp = new long[n + 2];
            cdp[n + 1] = n;
            for (int bcnt = 1; bcnt <= n; bcnt++)
            {
                long acnt = adp[bcnt - 1];
                while(acnt < n && a[acnt] < b[bcnt - 1])
                {
                    acnt++;
                }
                adp[bcnt] = acnt;
            ...
            for (int bcnt = n; bcnt > 0; bcnt--)
            {
                long ccnt = cdp[bcnt + 1];
                while (ccnt >0 && c[ccnt - 1] > b[bcnt - 1])
                {
                    ccnt--;
                }
                cdp[bcnt] = ccnt;
