...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            for (int waterB = 0; waterB < f; waterB+=b)
            {
                for (int waterA = 0; waterB + waterA < f; waterA+=a)
                {
                    int tmpWater = waterB + waterA;
                    if (tmpWater == 0) continue;
                    for (int sugarD = 0; tmpWater + sugarD <= f; sugarD+=d)
                    {
                        for (int sugarC = 0; tmpWater + sugarD + sugarC <= f; sugarC += c)
                        {
                            int tmpSugar = sugarD + sugarC;
                            if (((double)e) * tmpWater / 100 < tmpSugar)
                            {
                                break;
                            }
                            if (noudo(tmpWater, tmpSugar) > ansRate)
                            {
                                ansWater = tmpWater;
                                ansSugar = tmpSugar;
                                ansRate = noudo(tmpWater, tmpSugar);
                                //Console.WriteLine("{0} {1}", ansWater + ansSugar, ansSugar);
        ...
        double noudo(int water,int satou)
        {
            if (water == 0 && satou == 0) return 0;
            else if (water == 0) return -1;
            return ((double)100) * satou / (water + satou);
        }
    }
}
class Program
