...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            int[,] map = new int[n, n];
            long ans = 0;
            PriorityQueue<MyEdge> pq = new PriorityQueue<MyEdge>();
            for (int i = 0; i < n; i++)
            {
                for (int j = 0; j < n; j++)
                {
                    ...
                    if (i < j)
                    {
                        pq.Enqueue(new MyEdge(i, j, map[i, j]));
                    }
                }
            }
            UndirectedGraph<int, long> g = new UndirectedGraph<int, long>(n);
            while(pq.Count>0)
            {
                MyEdge e = pq.Dequeue();
                long len = g.ShortestPathLengthFrom(e.a, _ => _)[e.b];
                if (len >e.cost)
                {
                    g.AddEdge(new EdgeInfo<long>(e.a, e.b, e.cost));
                    ans += e.cost;
                }
                else if(len < e.cost)
                {
...
class MyEdge : System.IComparable
{
    public int a;
    public int b;
    public int cost;
    ...
    public int CompareTo(object obj)
    {
        ...
        //Priceを比較する
        return this.cost.CompareTo(((MyEdge)obj).cost);
        //または、次のようにもできる
