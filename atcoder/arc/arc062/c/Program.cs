    ...
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            for (int top = 0; top < n; top++)
            {
                ...
                {
                    long tmp = 1;
                    for (int turntmp = 0; turntmp < 4; turntmp++)
                    {

                        int[][] sokumen = new int[4][];
                        sokumen[0] = PanelSort(new int[4] {
                                c[top][Mod4(1)],
                                c[top][Mod4(0)],
                                c[bottom][Mod4(1+turntmp)],
                                c[bottom][Mod4(0+turntmp)] });
                        sokumen[1] = PanelSort(new int[4] {
                                c[top][Mod4(2)],
                                c[top][Mod4(1)],
                                c[bottom][Mod4(0+turntmp)],
                                c[bottom][Mod4(3+turntmp)] });
                        sokumen[2] = PanelSort(new int[4] {
                                c[top][Mod4(3)],
                                c[top][Mod4(2)],
                                c[bottom][Mod4(3+turntmp)],
                                c[bottom][Mod4(2+turntmp)] });
                        sokumen[3] = PanelSort(new int[4] {
                                c[top][Mod4(0)],
                                c[top][Mod4(3)],
                                c[bottom][Mod4(2+turntmp)],
                                c[bottom][Mod4(1+turntmp)] });
                        ...
                        for (int i = 0; i < 4; i++)
                        {
                            int sokutmp = map[sokumen[i]];
                            if (c[top] == sokumen[i]) sokutmp--;
                            if (c[bottom] == sokumen[i]) sokutmp--;
                            if (sokutmp < 0) sokutmp = 0;
                            sokutmp *= TurnCnt(sokumen[i]);
                            tmp *= sokutmp;
                        }
        ...
        int TurnCnt(int[] p)
        {
            if (p[0] == p[1] && p[0] == p[2] && p[0] == p[3]) return 4;
            if (p[0] == p[2] && p[1] == p[3]) return 2;
            return 1;
        }
    }
}
