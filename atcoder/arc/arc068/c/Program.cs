    ...
    class Problem : IDisposable
    {
        ...
        void Read()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                r[i] = sc.nextInt();
                ls[1].Add(i);
                if (l[i] != r[i] || (l[i] & 1) == 0) ls[2].Add(i);
            }
            for (int train = 3; train <= m; train++)
            {
                
                if((train & 1) == 0 && ((train / 2) & 1)!=0) //guusuu
                {
                    int waru2 = train / 2;
                    int cnt2 = 0;
                    int cnt3 = 0;
                    List<int> tmp = new List<int>();
                    while (ls[2].Count > cnt2 && ls[waru2].Count > cnt3)
                    {
                        if (ls[2][cnt2] == ls[waru2][cnt3])
                        {
                            tmp.Add(ls[2][cnt2]);
                            cnt2++;
                            cnt3++;
                        }
                        else if (ls[2][cnt2] > ls[waru2][cnt3])
                        {
                            cnt3++;
                        }
                        else
                    ...
                    {
                        if (l[i] % train == 0 || l[i] - l[i] % train + train <= r[i])
                        {
                            ls[train].Add(i);
                        }
                    }
