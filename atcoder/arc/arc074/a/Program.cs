...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            int tmph, tmpw, tmpmax, tmpmin;
            // h kara kiru
            tmph = h / 3;
            tmpmax = Math.Max(tmph * w, (h - tmph) * ((w + 1) / 2));
            tmpmin = Math.Min(tmph * w, (h - tmph) * (w / 2));
            ...
            tmpw = w / 3;
            tmpmax = Math.Max(tmpw * h, (w - tmpw) * ((h + 1) / 2));
            tmpmin = Math.Min(tmpw * h, (w - tmpw) * (h / 2));
            ans = Math.Min(ans, tmpmax - tmpmin);
