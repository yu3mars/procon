...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            long ans = 0;
            var xi = xs.Zip(index, (x, y) => new { x, y }).ToArray();
            Array.Sort(xi, (a, b) => {
                if (a.x != b.x) return a.x.CompareTo(b.x);
                return -a.y.CompareTo(b.y);
            });
            var yi = ys.Zip(index, (x, y) => new { x, y }).ToArray();
            Array.Sort(yi, (a, b) => {
                if (a.x != b.x) return a.x.CompareTo(b.x);
                return -a.y.CompareTo(b.y);
            });

            PriorityQueue<MyEdge> p = new PriorityQueue<MyEdge>();
            for (int i = 0; i < n-1; i++)
            {
                int weight = xi[i + 1].x - xi[i].x;
                MyEdge e = new MyEdge(xi[i + 1].y, xi[i].y, weight);
                p.Enqueue(e);

            }
            for (int i = 0; i < n - 1; i++)
            {
                int weight = yi[i + 1].x - yi[i].x;
                MyEdge e = new MyEdge(yi[i + 1].y, yi[i].y, weight);
                p.Enqueue(e);
            ...
            {
                MyEdge e = p.Dequeue();
                if(u.GetRootOf(e.a) != u.GetRootOf(e.b))
                {
                    u.UniteCategory(e.a, e.b);
                    ans += e.cost;
                    cnt++;
                }
            }
...
class MyEdge:IComparable
{
    public int a, b, cost;
    public MyEdge(int a, int b, int cost)
    {
        this.a = a;
        this.b = b;
        this.cost = cost;
    ...
    {
        return cost.CompareTo(((MyEdge)obj).cost);
    }
}
