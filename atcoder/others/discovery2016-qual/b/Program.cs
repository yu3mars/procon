...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            List<int> valls = new List<int>();
            for (int i = 0; i < n; i++)
            {
                if(dic.ContainsKey(a[i]) == false)
                {
                    dic.Add(a[i], new List<int>());
                    valls.Add(a[i]);
                }
                dic[a[i]].Add(i);
            }
            ...
            for (int i = 0; i < valls.Count; i++)
            {
                var nowls = dic[valls[i]];
                int min = -1;
                int max = -1;
                bool loop = false;
                for (int j = 0; j < nowls.Count; j++)
                {
                    if(nowls[j]<nowRoom)
                    {
                        min = Math.Max(min, nowls[j]);
                        loop = true;
                    }
                    else
                    {
                        max = Math.Max(max, nowls[j]);
                    }
                }
                if(loop)
                {
            ...
            if (nowRoom != 0) ansLoop++;
            Console.WriteLine(ansLoop);
        }
    }
}
class Program
