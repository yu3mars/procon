...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            Array.Sort(ls);
            string ans = ls[0];
            for (int i = 0; i < n; i++) //muri hantei
            {
                if (used[i] == true)
                {
                    if (s[i].Substring(0, Math.Min(s[i].Length, ans.Length)) != ans)
                    {
                        Console.WriteLine(-1);
                        return;
                    }
            ...
            Console.WriteLine(ls[0].Substring(0, ub));
        }
    }
}
class Program
