...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            long[,] dp = new long[n + 1, k + 1];//ichi,nageta
            for (int ichi = 1; ichi <= n; ichi++)
            {
                for (int nage = 1; nage <= k; nage++)
                {
                    dp[ichi, nage] = Math.Max(dp[ichi - 1, nage], dp[ichi - 1, nage - 1] + nage * a[ichi - 1]);
                }
            }
