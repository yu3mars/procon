...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        // int a;
        int h, w, a, b;
        long tatecnt, yokocnt, bothcnt, scnt, ans;
        string[] m;
        bool[,] tate, yoko, both;
        /// <summary>
        /// 読み込み処理をここに書く
        ...
        void SolveOne()
        {
            ...
            long tmp1 = (tatecnt - bothcnt) * a / 2;
            if (tatecnt == scnt) tmp1 -= a;
            long tmp2 = (yokocnt - bothcnt) * b / 2;
            if (yokocnt == scnt) tmp2 -= b;

            ans += Math.Max(tmp1, tmp2);
            if (tmp1 >= tmp2 && tatecnt == scnt && tatecnt!=bothcnt) kuzushi = true;
            if (tmp1 <= tmp2 && yokocnt == scnt && yokocnt!=bothcnt) kuzushi = true;

            long tmp3 = bothcnt * (a * 2 + b) / 4;
            if (kuzushi == false && bothcnt == scnt) tmp3 -= a;
            long tmp4 = bothcnt * (a + b * 2) / 4;
            if (kuzushi == false && bothcnt == scnt) tmp4 -= b;
