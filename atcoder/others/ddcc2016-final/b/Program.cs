...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            for (int i = 1; i < n; i++)
            {
                double h = Math.Abs(r - 2.0 * r / n * i);
                cut[i] = Math.Sqrt(r * r - h * h) * 2;
            }
            int center = n / 2;
            if (n % 2 == 0)
            {
                for (int i = 0; i <= center - m; i++)
                {
                    ans += GetCutLen(i);
                }
                for (int i = center - m + 1; i <= center; i++)
                {
                    ans += DualCut(i, i + m);
                }
            }
        ...
        double GetCutLen(int i)
        {
            if (i <= 0 || i >= n) return 0;
            return cut[i];
        }
        double DualCut(int i,int j)
        {
            return Math.Max(GetCutLen(i), GetCutLen(j));
        }
    }
}
