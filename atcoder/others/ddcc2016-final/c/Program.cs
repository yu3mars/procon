...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            long tmp = long.MaxValue / 2;
            char chr = s[0];
            List<long> ls = new List<long>();
            List<long> imos = new List<long>();
            List<bool> isOne = new List<bool>();
            long lscnt = 0;
            for (int i = 0; i < s.Length; i++)
            {
                ...
                {
                    ls.Add(lscnt);
                    if (s[i - 1] == '1') isOne.Add(true);
                    else isOne.Add(false);                   
                    if(imos.Count>0)
                    {
                        imos.Add(imos.Last() + ls.Last());
                    }
                    else
                    {
                        imos.Add(lscnt);
            ...
            {
                tmp = a * imos[i] + b * (n - imos[i]) + c * Math.Max(i, changeCnt - i - 1);
                if (i < changeCnt - i)
                {
                    if (s[(int)n - 1] == '0') tmp += c;
                }
                else
                {
                    if (s[0] == '1') tmp += c;
