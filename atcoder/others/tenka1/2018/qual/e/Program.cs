    ...
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            */
            ...
            List<int>[][] ls = new List<int>[90310][];
            for (int i = 0; i < ls.Length; i++)
            {
                ls[i] = new List<int>[601];
            ...
            for (int i = 0; i < ls.Length; i++)
            {
                for (int j = 0; j < ls[i].Length; j++)
                {
                    for (int k = 0; k < ls[i][j].Count; k++)
                    {
                        for (int l = k+1; l < ls[i][j].Count; l++)
                        {
                            for (int m = 0; m < ls[k][j].Count; m++)
                            {
                                if(ls[k][j][m] == l)
                                {
                                    ans++;
        ...
        {
            int ret = 0;
            ret = Math.Abs(x[i] / 300 - x[j] / 300) + Math.Abs(x[i] % 300 - x[j] % 300);
            return ret;
        }
        bool IsSameDistance(int i,int j, int k)
        {
            if(Manhattan(i,j) == Manhattan(j,k) && 
                Manhattan(i,j) == Manhattan(i,k))
            {
                return true;
            }
            return false;
