...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            long[] dpScore = new long[n + 1];   //score(n=used)
            long[] dpLast = new long[n + 1];   //LastUsed(n=used)
            ...
            {
                for (int kidouCnt = tCnt; kidouCnt >=0 ; kidouCnt--)
                {
                    long deltaScore = Math.Min(x, t[tCnt] - dpLast[kidouCnt]);
                    if(dpScore[kidouCnt+1] < dpScore[kidouCnt] + deltaScore)
                    {
                        dpScore[kidouCnt + 1] = dpScore[kidouCnt] + deltaScore;
                        dpLast[kidouCnt + 1] = t[tCnt];
                    }
                }
            }
            for (int i = 1; i <= n; i++)
            {
                Console.WriteLine(dpScore[i]);
            }
        }
    }
}
