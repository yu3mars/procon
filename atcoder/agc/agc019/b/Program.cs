...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            /*
            for (int mid = 0; mid < n; mid++) //xax
            {
                int haba = 1;
                while (isok(mid - haba) && isok(mid + haba))
                {
                    if (a[mid - haba] != a[mid + haba]) break;
                    haba++;
                    ans--;
                }
            }
            for (int mid = 0; mid < n - 1; mid++)   //xaax
            {
                int haba = 0;
                while (isok(mid - haba) && isok(mid + 1 + haba))
                {
                    if (a[mid - haba] != a[mid + 1 + haba]) break;
                    haba++;
                    ans--;
                }
            }
