...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            long minus = -long.MaxValue / 9; // kuwaeru minus
            long plus = long.MaxValue / 9;  // kuwaenai plus
            long mintmp = 0;
            long plutmp = 0;
            for (int i = 0; i < k; i++)
            {
            ...
            minus = Math.Max(minus, mintmp);
            plus = Math.Min(plus, plutmp);
            for (int i = 0; i < n - k; i++)
            {
                if (a[i] < 0) mintmp -= a[i];
                else plutmp -= a[i];

                if (a[i + k] < 0) mintmp += a[i + k];
                else plutmp += a[i + k];

                minus = Math.Max(minus, mintmp);
                plus = Math.Min(plus, plutmp);
            }
            long ans = Math.Max(sum + minus, sum - plus);
            ans = Math.Max(ans, 0);
