...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            long term = sum / termsum;
            if (sum % termsum != 0 || min < term || max > term * n)
            {
                Console.WriteLine("NO");
                return;
            }
            long loopcnt = 0;
            long basediff = a[0] - term;
            for (int i = 0; i < n; i++)
            {
                long basic = term * (i + 1);
                long diff = a[i] - basic;
                if((basediff -diff)%n!=0)
                {
                    ok = false;
                    break;
                }
                long loop = (basediff - diff) / term;
                if (loop > loopcnt) loopcnt = loop;
                else if(loop<loopcnt)
                {
                    ok = false;
                    break;
                }
