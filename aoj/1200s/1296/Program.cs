    ...
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            for (;;)
            {
                ...
                int ans = dfs(start, 0);
                if (ans == inf) Console.WriteLine(-1);
        ...
        int dfs(string s, int cnt)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                string next = s.Replace(ab[i][0], ab[i][1]);
                if (next == goal) ans = Math.Min(ans, cnt + 1);
                else if (goal.Length < next.Length || set.Contains(next)) ans = Math.Min(ans, inf);
                else
                {
                    ans = Math.Min(ans, dfs(next, cnt + 1));
