    ...
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            for (;;)
            {
                ...
                long aAns = Inf;
                long bAns = Inf;
                for (long aCnt = 0; aCnt < 100000; aCnt++)
                {
                    long aw = a * aCnt;
                    if ((aw + d) % b == 0)
                    {
                        long bCnt = (aw + d) / b;
                        if ((aCnt + bCnt < aAns + bAns) ||
                            ((aCnt + bCnt == aAns + bAns) && (aCnt * a + bCnt * b < aAns * a + bAns * b)))
                        {
                            aAns = aCnt;
                            bAns = bCnt;
                        }
                    }
                    if (Math.Abs(aw - d) % b == 0)
                    {
                        long bCnt = Math.Abs(aw - d) / b;
                        if ((aCnt + bCnt < aAns + bAns) ||
                            ((aCnt + bCnt == aAns + bAns) && (aCnt * a + bCnt * b < aAns * a + bAns * b)))
                        {
                            aAns = aCnt;
                ...
                Console.WriteLine("{0} {1}", aAns, bAns);
            }
        }
    }
}
