    ...
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            for (;;)
            {
                ...
                int h = sc.nextInt();
                if (n == 0) return;
                DirectedGraph<int, int> g = new DirectedGraph<int, int>(n * (m + 1));
                int[] cool = sc.nextInt(l);
                for (int i = 0; i < l; i++)
                {
                    for (int nowtime = 1; nowtime <= m; nowtime++)
                    {
                        g.AddEdge(cool[i] + nowtime * n, cool[i] + (nowtime - 1) * n, 1);
                ...
                for (int i = 0; i < k; i++)
                {
                    ...
                    int y = sc.nextInt();
                    int t = sc.nextInt();
                    for (int nowTime = 0; nowTime <= m - t; nowTime++)
                    {
                        g.AddEdge(x, y + t * n, t);
                        g.AddEdge(y, x + t * n, t);
                    }
                }
                var len = g.DijkstraFrom(a, _ => _);
                int ans = Func.Inf;
                for (int i = 0; i <= m; i++)
                {
                    ans = Math.Min(ans, len[h + i * n]);
                }
                if (ans < Func.Inf)
                {
