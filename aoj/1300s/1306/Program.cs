        ...
        void SolveOne()
        {
            ...
            for(;;)
            {
                ...
                for (int num = 0; num < n; num++)
                {
                    int befTime;
                    if (num == 0) befTime = 0;
                    else befTime = t[num - 1];
                    ok = false;
                    for (int carry = 0; carry <= 3; carry++)
                    {
                        if (dp[num, carry] == inf) continue;
                        //chokusetsu mukau
                        int nowTime = befTime + Math.Abs(p[num] - place) * (carry + 1);
                        if (carry <3 && nowTime <= t[num])
                        {
                            ok = true;
                            dp[num + 1, carry + 1] = Math.Min(dp[num + 1, carry + 1], dp[num, carry] + Math.Abs(p[num] - place));
                        }
                        //ichido orosu
                        nowTime = befTime + place * (carry + 1) + p[num];
                        if (nowTime <= t[num])
                        {
                            ok = true;
                            dp[num + 1, 1] = Math.Min(dp[num + 1, 1], dp[num, carry] + p[num] + place);
                        }
                    }
                    place = p[num];
                    if (ok == false)
                    {
                        Console.WriteLine("NG {0}", num + 1);
                        break;
                    }
                }
                if(ok)
                {
                    ...
                    Console.WriteLine("OK {0}", ans + p[n - 1]);
                }
