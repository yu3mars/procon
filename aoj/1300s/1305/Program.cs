        ...
        void SolveOne()
        {
            for(;;)
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    string key = str[0];
                    string[] val = str[1].Split(cs, StringSplitOptions.RemoveEmptyEntries);
                    dic.Add(key, val);
                    check.Add(key, false);
                    if (i == 0) first = key;
                }
                Console.WriteLine(dfs(first).Count);
            }
        }
        HashSet<string> dfs (string key)
        {
            HashSet<string> ls = new HashSet<string>();
            if (check[key]) return ls;
            check[key] = true;
            foreach (var s in dic[key])
            {
                if (dic.ContainsKey(s))
                {
                    foreach (var ss in dfs(s))
                    {
                        ls.Add(ss);
                    }
                }
