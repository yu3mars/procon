...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        {
            int n = sc.nextInt();
            bool[] isYes = new bool[n];
            bool[] check = new bool[n];
            int[] len = new int[n];
            int ysum = 0;
            int chkcnt = 0;
            int ans = 0;
            List<int>[] ls = new List<int>[1000];
            ...
            for (int nowl = 0; nowl < ls.Length; nowl++)
            {
                for (int nowy = 0; nowy < ls[nowl].Count; nowy++)
                {
                    int nowNum = ls[nowl][nowy];
                    if (check[nowNum]) continue;
                    ans++;
                    check[nowNum] = true;
                    chkcnt++;
