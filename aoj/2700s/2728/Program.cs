...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            ans = 0;
            a = long.Parse(sstr);
            dfs(new StringBuilder(), 0, new bool[10]);
            Console.WriteLine(sans);
        }

        void dfs(StringBuilder sb, int cnt,bool[] used)
        {
            if(cnt==n)
            {
                long b = long.Parse(sb.ToString());
                long tmp = Math.Min(Math.Abs(a - b), ((long)Math.Pow(10, n)) - Math.Abs(a - b));
                if(ans<tmp)
                {
                    ans = tmp;
                    sans = sb.ToString();
