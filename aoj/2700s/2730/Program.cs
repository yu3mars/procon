    ...
    class Problem : IDisposable
    {
        ...
        void SolveOne()
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (s[i] == '<')
                {
                    if (lArray[i] >= rArray[i]) //migi kara deru
                    {
                        tmp = lls[Math.Min(lls.Count - 1, lArray[i] - rArray[i])];
                    }
                    else
                    {
                        tmp = n - rls[rArray[i] - lArray[i] - 1];
                    }
                }
                else
                {
                    if (rArray[i] >= lArray[i]) //hidari kara deru
                    {
                        tmp = n - rls[Math.Min(rls.Count - 1, rArray[i] - lArray[i])];
                    }
                    else
                    {
                        tmp = lls[lArray[i] - rArray[i] - 1];
                    }
                }
                if (tmp == 916)
                {
...
namespace MyIO
{
    ...
    class Scanner : IDisposable
    {
        bool isConsole;
        TextReader file;
        public Scanner() { Console.SetIn(new StreamReader(Console.OpenStandardInput(100100))); file = Console.In; }
