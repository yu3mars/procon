...
namespace Tmp
{
    class Problem : IDisposable
    {
        ...
        struct Cake
        {
            ...
            public Cake[] divide(int n)
            {
                Cake[] ret = new Cake[2];
                n %= (w + h);
                if (n < w)
                {
                    int minW = Math.Min(n, w - n);
                    int maxW = w - minW;
                    ret[0] = new Cake(minW, h);
                    ret[1] = new Cake(maxW, h);
                }
                else
                {
                    n -= w;
                    int minH = Math.Min(n, h - n);
                    int maxH = h - minH;
                    ret[0] = new Cake(w, minH);
                    ret[1] = new Cake(w, maxH);
                }
                return ret;
            }
        }
